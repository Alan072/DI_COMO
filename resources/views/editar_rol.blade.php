...
@section('contenido')
    ...
    <div style="margin-left: 20px; margin-top: -60px;">
        ...
        <div class="bg-white rounded-lg shadow p-6" style="width: 50%; margin-left: 20px; margin-top: -20px;">
            <form method="post" action="{{ route('update_rol', $rol->id_rol) }}">
                @csrf
                {!! method_field('PUT') !!} @csrf
                <label>ID</label>
                <input type="number" name="id_rol" value="{{ $rol->id_rol }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    style="width: 100%;" placeholder="1" required>
                <label>Nombre del rol</label><br>
                <input type="text" name="nombre_rol" value="{{ $rol->nombre_rol }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    style="width: 100%;" placeholder="Nombre del rol" required>
                <br>
        </div>
        <div>
            <button type="submit"
