...
@section('contenido')
                ...
                @foreach ($almacen as $item)
                    <tr
                        ...
                        <td class="flex items-center px-6 py-4 space-x-3">
                            ...
                            <form id="eliminar_alm_form_{{ $item->id_almacen }}"
                                action="{{ route('eliminar_almacen', ['id_almacen' => $item->id_almacen]) }}" method="POST"
                                class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="button"
                                    onclick="confirmarEliminacion('eliminar_alm_form_{{ $item->id_almacen }}')"
    ...
    <script>
        function confirmarEliminacion(formId) {
            Swal.fire({
                title: '¿Está seguro de que desea eliminar este almacen?',
                icon: 'warning',
